...
1.  Перейдите на страницу регистрации приложений: [yoomoney.ru/myservices/new](https://yoomoney.ru/myservices/new)
2.  Заполните форму:
    *   **Название**: Любое (например, `Vless VPN Bot`).
    *   **Адрес сайта**: `https://ВАШ_ДОМЕН` (например, `https://vless.24x7.hk`).
    *   **Redirect URI**: `https://ВАШ_ДОМЕН/yoomoney/callback` (ОБЯЗАТЕЛЬНО!).
    *   **Проверять подлинность приложения**: Поставьте галочку, если хотите использовать `Client Secret` (рекомендуется).

### Шаг 2: Настройка в Админ-панели
1.  Зайдите в админку бота -> **Настройки** -> **Платежи**.
2.  Включите **YooMoney**.
3.  Заполните поля:
    *   **Кошелёк YooMoney**: Номер вашего кошелька (41001...).
    *   **Client ID**: Скопируйте из созданного приложения.
    *   **Redirect URI**: Тот же, что указали при регистрации (`https://ВАШ_ДОМЕН/yoomoney/callback`).
    *   **Client Secret**: Скопируйте из приложения (если включали проверку подлинности).
4.  Нажмите **Сохранить**.
5.  Нажмите кнопку **"Подключить YooMoney (OAuth)"**. Вас перекинет на сайт ЮMoney для подтверждения прав. После успеха вы вернетесь в админку с сохраненным токеном.

### Шаг 3: Настройка уведомлений (для автоматического зачисления)
1.  Зайдите в настройки кошелька ЮMoney -> [HTTP-уведомления](https://yoomoney.ru/transfer/myservices/http-notification).
2.  Включите уведомления.
3.  В поле URL вставьте: `https://ВАШ_ДОМЕН/yoomoney-webhook`.
4.  Нажмите "Показать секрет" и скопируйте его.
5.  Вставьте этот секрет в админке бота в поле **"Секрет вебхука YooMoney"**.
...
Если при `git pull` вы видите ошибку:
`error: Your local changes to the following files would be overwritten by merge: users.db`

**Решение (на сервере):**
```bash
cp users.db users.db.bak   # Резервная копия
git checkout users.db      # Сброс конфликтующего файла
git pull                   # Обновление
cp users.db.bak users.db   # Восстановление базы
rm users.db.bak            # Удаление копии
```
