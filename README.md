...
В проекте реализована поддержка **9 платёжных систем**, охватывающих фиатные деньги (рубли), криптовалюты и внутренние валюты Telegram. Все системы настраиваются через **Веб‑панель → Настройки → Платёжные системы**.
...
### Ошибка `KeyError: 'ContainerConfig'` при запуске
Возникает из-за устаревшей версии `docker-compose` (часто 1.29.2).
**Решение:** Удалите контейнеры вручную и запустите заново.

1. Посмотрите список всех контейнеров:
   ```bash
   sudo docker ps -a
   ```
2. Найдите ID контейнеров (первая колонка), связанных с `shopbot` или `nginx`.
3. Удалите их по ID (замените `CONTAINER_ID` на реальные цифры/буквы):
   ```bash
...
### Ошибка `bind() to 0.0.0.0:2096 failed` (Nginx не запускается)
Это означает, что порт **2096** уже занят другой программой (скорее всего, вашей панелью 3x-ui). Nginx бота не может использовать тот же порт.

**Решение:** Смените порт для бота в конфиге Nginx.
1. Откройте конфиг (замените `my-3xui-shopbot` на имя вашей папки, если отличается):
   ```bash
   sudo nano /etc/nginx/sites-enabled/my-3xui-shopbot.conf
   ```
2. Найдите строку `listen 2096 ssl http2;` и замените **2096** на свободный порт (например, **8443**).
...
Если вы не знаете точное имя контейнера:
1. Найдите его имя:
   ```bash
   sudo docker ps
   ```
   (Ищите контейнер с образом `...-shopbot` и статусом `Up`).
2. Посмотрите логи (замените `NAME` на имя из колонки NAMES):
   ```bash
   sudo docker logs -f NAME
   ```
   Часто имя просто `3xui-shopbot`.

### Ошибка "Router is already attached" или бот завис
Если в логах вы видите ошибку `Router is already attached to...` или `Support-бот уже запущен`, это значит, что старый процесс бота не завершился корректно. Вам нужно найти и убить старый процесс вручную.

**Решение (SSH):**

1. **Найдите процесс бота:**
   ```bash
   ps aux | grep shop_bot
   ```
   *Вы увидите список процессов. Ищите тот, который запущен через python.*

2. **Убейте зависший процесс:**
   ```bash
   pkill -f shop_bot
   ```
   *Или используйте `kill -9 PID`, где PID — идентификатор процесса из первого шага.*
