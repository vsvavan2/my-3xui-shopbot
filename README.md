...
### Вариант 1: Стандартное обновление (если уже стоит этот мод)

```bash
curl -sSL https://raw.githubusercontent.com/vsvavan2/my-3xui-shopbot/main/install.sh | sudo bash
```

### Вариант 2: Миграция с другой версии (сохранение данных)

Если у вас стоит другая версия бота, и вы хотите обновиться на эту без потери пользователей и настроек:

1. **Зайдите в папку бота** (обычно `3xui-shopbot`):
   ```bash
   cd 3xui-shopbot
   ```

2. **Остановите бота и сделайте бэкап:**
   ```bash
...
3. **Смените репозиторий и обновите код:**
   ```bash
...
*Если что-то пошло не так, восстановите `users.db` из бэкапа.*
